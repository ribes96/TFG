...
class Demo10(Demo):
    ...
    def __init__(self):
        ...
        # self.gui = Tab()
        # Para contener VBox con lo que toca
        self.models_tab = Tab()

        # Para contener diccionarios con los widgets
        self.models = []
        ...
        # self.models_tab.children += (self.get_model_tab(),)
        self.add_model_tab()
        ...
        #                      self.tab4]
        # self.models.children = [self.tab0, self.tab1, self.tab2, self.tab3,
        #                      self.tab4]
        ...
        self.mod_add_bt.on_click(self.add_model_tab)
        # self.mod_remove_bt.on_click(self.remove_model_bar)
        self.mod_remove_bt.on_click(self.remove_model_tab)
    ...
    def set_tab_titles(self):
        for i, tab in enumerate(self.models):
            name = tab['model'].value
            self.models_tab.set_title(i, name)

    def get_model_tab(self):
        '''
        Returns a tab to choose a model
        '''
        model_selector = self.get_default_model_selector()
        model_selector.description = 'Model'
        sampler_selector = self.get_default_sampler_selector()
        sampler_selector.description = 'Sampler'
        gamma_selector = self.get_default_gamma_selector()
        gamma_selector.description = 'Gamma'
        box_type_selector = self.get_default_box_type_selector()
        box_type_selector.description = 'Box Type'
        n_estim_selector = self.get_default_n_estimators_selector()
        n_estim_selector.description = 'N. estim.'
        dt_mid_selector = FloatSlider(value=.0, min=0, max=1, step=.1,
                                      description='min_impurity_decrease:')
        logit_C_selector = FloatLogSlider(value=1000, base=10, min=-5,
                                          max=5, step=1, description='C')
        linear_svc_C_selector = IntSlider(value=5, min=1, max=10, step=1,
                                          description='C')

        # lay = Layout(width='800px')
        lay_width = '800px'

        dt_mid_selector.style = {'description_width': 'initial'}

        # gamma_selector.layout = lay
        gamma_selector.layout.width = lay_width

        # n_estim_selector.layout = lay
        n_estim_selector.layout.width = lay_width
        # dt_mid_selector.layout = lay
        dt_mid_selector.layout.width = lay_width
        # logit_C_selector.layout = lay
        logit_C_selector.layout.width = lay_width
        # linear_svc_C_selector.layout = lay
        linear_svc_C_selector.layout.width = lay_width

        def tab_changed(*args):
            self.set_tab_titles()
            if sampler_selector.value == 'None':
                gamma_selector.layout.visibility = 'hidden'
            else:
                gamma_selector.layout.visibility = 'visible'

            if box_type_selector.value == 'None':
                n_estim_selector.layout.visibility = 'hidden'
            else:
                n_estim_selector.layout.visibility = 'visible'

            model = model_selector.value
            if model == 'dt':
                dt_mid_selector.layout.visibility = 'visible'
                dt_mid_selector.layout.display = 'flex'
                logit_C_selector.layout.visibility = 'hidden'
                logit_C_selector.layout.display = 'none'
                linear_svc_C_selector.layout.visibility = 'hidden'
                linear_svc_C_selector.layout.display = 'none'
            elif model == 'logit':
                dt_mid_selector.layout.visibility = 'hidden'
                dt_mid_selector.layout.display = 'none'
                logit_C_selector.layout.visibility = 'visible'
                logit_C_selector.layout.display = 'flex'
                linear_svc_C_selector.layout.visibility = 'hidden'
                linear_svc_C_selector.layout.display = 'none'
                ...
                logit_C_selector.layout.visibility = 'hidden'
                logit_C_selector.layout.display = 'none'
                linear_svc_C_selector.layout.visibility = 'visible'
                linear_svc_C_selector.layout.display = 'flex'

        sampler_selector.observe(tab_changed, 'value')
        box_type_selector.observe(tab_changed, 'value')
        model_selector.observe(tab_changed, 'value')
        tab_changed()

        dic = {
            'model': model_selector,
            'sampler': sampler_selector,
            'gamma': gamma_selector,
            'box_type': box_type_selector,
            'n_estim': n_estim_selector,
            'dt_params': dt_mid_selector,
            'logit_params': logit_C_selector,
            'linear_svc_params': linear_svc_C_selector,
        }

        box = VBox([model_selector, dt_mid_selector, logit_C_selector,
                    linear_svc_C_selector, sampler_selector, gamma_selector,
                    box_type_selector, n_estim_selector])

        return box, dic

    def add_model_tab(self, e=None):
        box, dic = self.get_model_tab()
        self.models_tab.children += (box,)
        self.models.append(dic)
        self.set_tab_titles()

    def remove_model_tab(self, e=None):
        # TODO borrar realmente los widgets
        if len(self.models) <= 1:
            return
        self.models_tab.children = self.models_tab.children[:-1]
        del self.models[-1]

    def models_gui_to_data(self, modic):
        '''
        Parameters
        modic : dic
            The dictionary of a model
        '''
        model_name = modic['model'].value
        sampler_name = modic['sampler'].value
        if sampler_name == 'None':
            sampler_name = 'identity'
        box_type = modic['box_type'].value
        if box_type == 'None':
            box_type = 'none'
        ret_dic = {
            'model_name': model_name,
            'sampler_name': sampler_name,
            'gamma': modic['gamma'].value,
            'box_type': box_type,
            'n_estim': modic['n_estim'].value,
        }

        if model_name == 'dt':
            ret_dic['base_model_params'] =\
                {'min_impurity_decrease': modic['dt_params'].value}
        if model_name == 'logit':
            ret_dic['base_model_params'] = {'C': modic['logit_params'].value}
        if model_name == 'linear_svc':
            ret_dic['base_model_params'] =\
                {'C': modic['linear_svc_params'].value}

        return ret_dic

    def models_gui_to_data2(self, i):
        '''
        Parameters
        ----------
        i : int
    ...
    def gui_to_data(self):
        models = [self.models_gui_to_data(d) for d in self.models]
        ret_dic = {
    ...
    # self.dt_min_weight_fraction_leaf_selector.value,
    #         'max_leaf_nodes': int(self.dt_max_leaf_nodes_selector.value),
    #         'min_impurity_decrease': self.dt_min_impurity_decrease_selector
    # .value,
    #     }
    #     logit_hp = {
    ...
    #     pca_order_selector.layout = Layout(width='150px',
    # visibility='hidden')
    ...
    #     '''
    #     Returns a widget (accordion) full of labels showing the info in d,
    # with
    #     the key in bold
    #     '''
    #     labels = [HTML(value='<strong>{0}</strong>: {1}'.format(k, d[k])) for
    # k in d]
    #     v = VBox(labels)
    #     ac = Accordion([v])
    ...
    def run_demo(self, dts_name, dts_size, features_range, models):
        ...
        warning in raised, self.ERRORS is filled with some info, and the
        execution continues with the rest of the models.
...
- Size: **{1}**
        ...
        for m in models:
            ...
            model_params = m['base_model_params']
            sampler_gamma = m['gamma']
            if box_type == 'none':
                n_estim = None
            ...
            #                 box_type=box_type)
            clf = get_model(model_name=model_name, model_params=model_params,
                            sampler_name=sampler_name, pca_bool=False,
                            pca_first=False, n_estim=n_estim,
            ...
            if sampler_name == 'identity':
                ...
                lab = self.get_label(model_name=model_name,
                                     sampler_name=sampler_name,
                                     box_type=box_type, n_estim=n_estim,
                                     pca=False, pca_first=False)
                train_score = {
                    'absi': features_range,
...
#     def run_demo(self, dts_name, dts_size, features_range, models):
#         '''
#         Just reading from the arguments, returns a pair of list of
#  dictionarys,
